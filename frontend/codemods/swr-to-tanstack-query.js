...
function extractKeyAndEnabled(j, keyArg) {
  if (keyArg.type === 'ConditionalExpression') {
    ...
    if (isNullAlternate) {
      const enabledCondition = j.unaryExpression('!', j.unaryExpression('!', test));
...
function buildQueryFn(j, fetcherArg, queryKey) {
  if (fetcherArg.type === 'Identifier') {
    return j.arrowFunctionExpression([], j.callExpression(fetcherArg, [queryKey]));
  }

  if (fetcherArg.type === 'ArrowFunctionExpression') {
    return fetcherArg;
  }
...
function transformMutateToRefetch(j, root, mutateVars) {
  ...
  mutateVars.forEach((varName) => {
    if (varName === 'mutate') {
      root.find(j.Identifier, { name: 'mutate' }).forEach((path) => {
        if (
        ...
        if (path.parent.node.type === 'CallExpression' && path.parent.node.callee === path.node) {
          return;
        }
